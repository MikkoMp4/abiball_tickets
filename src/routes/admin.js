...
router.post('/upload-pdf', pdfUpload.array('pdfs', 20), async (req, res) => {
  ...
  for (const file of req.files) {
    ...
    for (const tx of transactions) {
      ...
      // Person über Code-Teil der Referenz (ABIBALL-XXXXX) finden
      const codeMatch = ref.match(/ABIBALL-([A-Z0-9]{4,12})/);
      const code      = codeMatch ? codeMatch[1] : null;

      const person = code
        ? db.prepare('SELECT * FROM persons WHERE code = ?').get(code)
        : null;
      ...
      if (person && order && amountMatches && !order.paid) {
        ...
        db.prepare(
          'UPDATE orders SET paid = 1, paid_at = datetime(\'now\') WHERE id = ?'
        ).run(order.id);

        // Zahlung eintragen / aktualisieren
        ...
        if (!existingPayment) {
          ...
          db.prepare(
            'UPDATE payments SET matched = 1, person_id = ? WHERE id = ?'
          ).run(person.id, existingPayment.id);
        }
        markedPaid = true;
      }
      ...
      allResults.push({
        ...
        expectedAmount,
        amountMatches,
        alreadyPaid:    order ? !!order.paid : false,
        markedPaid,
      });
    }
    ...
    if (transactions.length === 0) {
      allResults.push({
        file: file.originalname,
        error: 'Keine Abiball-Referenzen im PDF gefunden',
      });
    }
  }
